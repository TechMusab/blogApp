...
const postMatchesQuery = (post: RootState['posts']['posts'][number], query: string) => {
  ...
  return [post.title, post.author, post.category, ...(post.tags ?? [])].some((value) =>
    normalize(value).includes(query)
  );
};

export const selectSearchResults = createSelector([selectPosts, selectQuery], (posts, query) => {
  ...
  const categories = [...counts.keys()].sort((a, b) => {
    ...
    return (
      (left === -1 ? Number.MAX_SAFE_INTEGER : left) -
        (right === -1 ? Number.MAX_SAFE_INTEGER : right) || a.localeCompare(b)
    );
  });
  return [
